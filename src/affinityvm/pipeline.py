...
class RegressionHead(nn.Module):
    ...
    def __init__(self, latent_dim: int = LATENT_DIM, dropout: float = 0.15):
        super().__init__()
        in_dim = latent_dim + 1   # +1 for MD energy feature
    ...
    def forward(self, latent: torch.Tensor, energy: torch.Tensor) -> torch.Tensor:
        e = energy.view(-1, 1) / 100.0   # rough normalisation (kcal/mol range)
        x = torch.cat([latent, e], dim=-1)
...
class AffinityPipeline(nn.Module):
    ...
    def __init__(self, config: Optional[PipelineConfig] = None):
        ...
        self._engine_caches: list = []   # prevent premature GC during backward
    ...
    def forward(self, smiles_list: list[str]) -> torch.Tensor:
        ...
        Returns:
            predictions: [B] float32 tensor of pIC50 values
        """
        ...
        # GNN → latent [B, 256]
        latents = self.gnn(batch)

        # MD energy for each molecule (iterate — each has different atom count)
        energies = []
        caches   = []
        for i, fo in enumerate(feat_outputs):
            latent_i = latents[i]   # [256], on device, differentiable
            pos_i    = torch.from_numpy(fo["positions"])   # [N, 3] CPU
            mas_i    = torch.from_numpy(fo["masses"])      # [N]    CPU
            e_i, cache_i = md_energy(
                latent_i, pos_i, mas_i,
        ...
        return self.head(latents, energy_tensor)            # [B]

    def predict(self, smiles_list: list[str]) -> np.ndarray:
        """
        Inference-only prediction. Returns numpy array of pIC50 values.
        """
        self.eval()
        with torch.no_grad():
